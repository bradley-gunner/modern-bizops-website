...
- **UTM registry: DONE, this line was stale, corrected 2026-08-25.** `utm_medium=nurture`, the `lm_welcome_scan` campaign row and both content rows were registered 2026-08-19 alongside PR #83. The `utm_content` stem discrepancy is also **resolved**: Bradley's call 2026-08-25 is `welcome_scan_0X_v1`, agreeing with the campaign slug and the product name, and the code now emits that via `UTM_CONTENT_STEM`. Nothing had shipped on the old value, so no in-place retirement is owed. **Still owed on any future Content.csv change:** re-sync the `REGISTERED_CONTENT` allowlist in `marketing-engine-dashboard.html`, which is a manual mirror the artifact cannot read from disk.
