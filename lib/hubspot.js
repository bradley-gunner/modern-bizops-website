...
// Distinguishes which lead magnet created/last-touched the contact. The magnet
// creates contacts server-side via the API, so HubSpot's native form-conversion
// fields stay blank; we set this explicitly instead. Created on demand by the
// scorecard route via ensureCustomContactProperties(), which reconciles the
// option set if the property already exists. NOTE: distinct from utm_content,
// which carries the asset-variant value.
//
// DO NOT REMOVE THE "Playbook" OPTION. /playbook was retired on 2026-08-18 and
// nothing writes that value any more, but contacts created before then still
// carry it. Because ensureCustomContactProperties() RECONCILES the option set,
// deleting the option here would strip it from the live HubSpot property on the
// next scorecard submission and orphan those contacts' values.
