...
// Enum options for scorecard_maturity_stage: the four Revenue Operations
// Maturity stages, keyed by placement.stage 1..4. Label === value so the stored
// value reads cleanly in reports and the email loop.
const MATURITY_STAGE_OPTIONS = ["Reactive", "Repeatable", "Predictable", "Compounding"].map(
  (name, i) => ({ label: name, value: name, displayOrder: i })
);
...
export const SCORECARD_RESULT_PROPERTIES = [
  ...
  {
    name: "scorecard_maturity_stage",
    label: "Scorecard Maturity Stage",
    description: "Revenue Operations Maturity stage placement.",
    type: "enumeration",
    fieldType: "select",
    groupName: SCORECARD_PROPERTY_GROUP.name,
    options: MATURITY_STAGE_OPTIONS,
  },
  {
