 ...
 *
 * Sync procedure (preserving site-local rows):
 ...
 *   3. For each metric except `grr`, copy the canonical row's values into the
 *      matching row here. NEVER copy the whole BUSINESS_MODEL_BENCHMARKS object
 *      verbatim - the `grr` rows are site-local and do not exist in canonical.
 *   4. Diff against the previous version. Verify all 8 `grr` rows are still
 *      present (the schema test in businessModelBenchmarks.test.js will fail
 *      if any are dropped).
 *   5. Update tests if any band edges shifted.
 *   6. Ship the version bump in the commit message so PR review surfaces it.
 *
...
const BUSINESS_MODEL_BENCHMARKS = {
  ...
  PROFESSIONAL_SERVICES: {
    ...
    metrics: {
      ...
      grr:                { direction: 'higher', median: 0.82,   range: [0.75, 0.90],     unit: 'ratio', source: 'SPI Research 2025 Professional Services Maturity Benchmark (84 percent client retention); Focus Digital agency benchmarks (75 to 85 percent), client retention as GRR proxy', asOf: 2025, confidence: 'estimated' },
    },
  },
  B2B_PRODUCT: {
    ...
    metrics: {
      ...
      grr:                { direction: 'higher', median: 0.78,   range: [0.68, 0.86],     unit: 'ratio', source: 'CustomerGauge B2B NPS and Customer Experience Benchmarks (manufacturing retention about 78 percent); anchored below professional services', asOf: 2025, confidence: 'estimated' },
    },
  },
  ECOMMERCE: {
    ...
    metrics: {
      ...
      grr:                { direction: 'higher', median: 0.30,   range: [0.20, 0.45],     unit: 'ratio', source: 'Matches nrr row (no consumer expansion to separate GRR from NRR); annual cohort revenue retention from Klaviyo repeat-purchase benchmarks (about 28 percent repeat rate) plus DTC averages. Note: Q15 (churn) is hidden for ECOMMERCE in v1, so this row is defensive and not user-facing today.', asOf: 2026, confidence: 'estimated' },
    },
  },
  B2C_SERVICES: {
    ...
    metrics: {
      ...
      grr:                { direction: 'higher', median: 0.70,   range: [0.55, 0.80],     unit: 'ratio', source: 'Matches nrr row (no consumer expansion to separate GRR from NRR); HFA 2025 Fitness Industry Benchmarking (66.4 percent client retention); IHRSA Profiles of Success (71.4 percent clubs, 80 percent PT studios)', asOf: 2026, confidence: 'estimated' },
    },
  },
  B2C_SUBSCRIPTION: {
    ...
    metrics: {
      ...
      grr:                { direction: 'higher', median: 0.60,   range: [0.45, 0.75],     unit: 'ratio', source: 'Matches nrr row (negligible consumer expansion makes GRR equal to NRR per Recurly); Recurly State of Subscriptions (median 4 percent monthly churn, 2200+ merchants; annualizes to about 0.60 GRR); RevenueCat State of Subscription Apps (median 42 percent of revenue retained at 12 months)', asOf: 2026, confidence: 'cited' },
    },
  },
  MARKETPLACE: {
  ...
  OTHER: {
    ...
    metrics: {
      ...
      grr:                { direction: 'higher', median: 0.78,   range: [0.65, 0.88],     unit: 'ratio', source: 'Weighted midpoint across B2B_SAAS, PROFESSIONAL_SERVICES, B2B_PRODUCT, and MARKETPLACE grr medians; no survey-grade SMB-wide GRR benchmark exists', asOf: 2025, confidence: 'estimated' },
    },
  },
};
