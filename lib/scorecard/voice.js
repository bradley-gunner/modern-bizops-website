...
export function bandMarker(composite) {
  return sanitizeVoice(
    `Scored ${composite} out of 5, based on what you told us about yourself.`
  );
}
...
export const MATH_LINES = {
  ...
  salesCycle: ({ cycleDisplay, cycleProv, medianDisplay, revenueDisplay, revenueProv }) =>
    sanitizeVoice(
      `The math: your ${cycleDisplay} cycle (${cycleProv}) against the ${medianDisplay} peer median. Closing to the median turns the same pipeline over faster, and the dollar line is that extra throughput applied to your ${revenueDisplay} revenue (${revenueProv}).`
    ),
