...
export function CTA_FOCUS_TEMPLATE(focusLabel) {
  return sanitizeVoice(
    `The ${AUDIT.name} starts where this Scan stopped guessing. It goes into your systems, measures your ${focusLabel} gap for real, and comes back with what to automate first and what it costs.`
  );
}

/** The same bridge with no competency to name, used when the Scan surfaced no
 *  single binding boundary. */
export const CTA_GENERIC_LINE = sanitizeVoice(
  `The ${AUDIT.name} starts where this Scan stopped guessing. It goes into your systems, measures the real picture, and comes back with what to automate first and what it costs.`
);
