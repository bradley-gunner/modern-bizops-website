...
export const DISCLOSURE = sanitizeVoice(
  'This is a directional read from fifteen questions. It tells you which stage of the maturity ladder you sit on, where you stack up against peer benchmarks on the three metrics I can compute from your inputs, and the boundary you need to cross next. It does not replace the full assessment, which connects to your CRM and revenue tools, scores all 44 competencies, and produces the specific roadmap from where you are to the business outcome you want. If the numbers above feel directionally right, that is the signal to take the next step.'
);
