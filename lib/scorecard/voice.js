 ...
 *
 * INVERTED 2026-08-12. This module used to say the opposite: the Scan surfaces
 * were the carve-out that stayed in Bradley's singular voice while the rest of
 * the site said "we", and sanitizeVoice() threw on \b(we|our|us)\b. Bradley
 * retired the singular as a brand voice that day, which left the sanitizer
 * throwing on the house voice. It now throws on the singular instead. Offer
 * copy in lib/offers.js was already plural, so it is no longer at odds with
 * this file; only the offer NAME and PRICE are still pulled from there.
 *
...
export const SECTION_SUBLINES = {
  ...
  3: sanitizeVoice('Three numbers about your business so we can put dollars on the gap. Bands, not exact figures.'),
};
...
export const DISCLOSURE = sanitizeVoice(
  `This is a directional read from fifteen questions you answered about yourself. It tells you which stage of the maturity ladder you sit on, where you stack up against peer benchmarks on the three metrics we can compute from your inputs, and the boundary you need to cross next. It does not replace the ${AUDIT.name}, which connects to your CRM and revenue tools, scores more than fifty competencies, and ranks what to automate first against the outcome you want. Where a number above looks wrong to you, that is because we asked for an estimate and you gave us one. The audit is how you replace the estimate with the real figure.`
);
...
export const CTA_LINES = [
  ...
  sanitizeVoice('More than fifty competencies scored, forty-four computed from those records and the newest seven on the call, plus an AI readiness profile'),
...
export const NO_GAP_HEADLINE = {
  lead: sanitizeVoice('Your numbers hold up against {model_label} peers. The gap we can see is an operational one.'),
  subline: sanitizeVoice('No defensible dollar gap from your inputs. Below is the read on where you sit operationally and the one boundary we would close next.'),
};
...
export function NO_GAP_BINDING(binding) {
  ...
  return sanitizeVoice(
    `What you told us about ${labels} is the boundary you need to cross next. Your numbers will not show it until you try to grow through it.`
  );
}
