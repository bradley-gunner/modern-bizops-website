...
export const DISCLOSURE = sanitizeVoice(
  `This is a directional read from fifteen questions you answered about yourself. It tells you which stage of the maturity ladder you sit on, where you stack up against peer benchmarks on the three metrics we can compute from your inputs, and the boundary you need to cross next. It does not replace the ${AUDIT.name}, which connects to your CRM and revenue tools, scores more than forty competencies, and ranks what to automate first against the outcome you want. Where a number above looks wrong to you, that is because we asked for an estimate and you gave us one. The audit is how you replace the estimate with the real figure.`
);
