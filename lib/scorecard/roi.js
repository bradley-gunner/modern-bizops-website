...
/**
 * Backstop, not a shaping tool. Until 2026-08-14 these caps were load-bearing:
 * the retired sales-cycle throughput model routinely produced figures larger
 * than the respondent's entire revenue, and the caps dragged them back to 75
 * percent of it, which is how a $10M business was told it was losing $7.5M.
 * With that generator gone, both surviving generators are gap-to-peer models
 * that cannot exceed revenue by construction, so these should now fire only on
 * genuinely extreme inputs. If you find them binding on an ordinary profile
 * again, treat that as evidence a generator is wrong rather than as the caps
 * doing their job.
 */
...
const BODY_BUILDERS = {
  revenuePerEmployee: (floor, median) =>
    `You are leaving ${lossRangePhrase(floor, median)} of annual revenue uncaptured every year, and it is already inside the team you have. Most founders in your position describe the opposite problem: every dollar of revenue growth requiring another hire.`,
...
/**
 * RETIRED as a dollar generator 2026-08-14. It computed
 * `(clientDays / peerMedian - 1) * revenue`, a throughput model asserting that
 * annual revenue scales linearly with pipeline turnover: on a 240-day cycle
 * against a 103-day median it claimed revenue would be 2.33x higher, i.e.
 * $13.3M "uncaptured" on a $10M business, which is 65 percent of what the caps
 * were quietly suppressing. Cycle length does not govern annual revenue; lead
 * supply, capacity and demand do.
 *
 * The row survives in the opportunity map as an evidence row (see
 * buildOpportunityMap) and in the peer comparison table, where the days-vs-days
 * benchmark is honest. Only the invented dollar figure is gone.
 */
...
export const generators = { revenuePerEmployee, retention };
...
/** Which computed metric feeds which menu area. salesCycle left this map on
 *  2026-08-14 when its dollar model was retired; follow-up and pipeline is now
 *  an evidence row built from the same benchmark, in days rather than dollars. */
...
export function buildOpportunityMap(answers, benchmark, observed) {
  ...
  const computedRows = [];
  for (const metricKey of ['revenuePerEmployee', 'retention']) {
    ...
    if (!line) {
      ...
      const input = metricKey === 'retention' ? resolveInput('q16', answers) : resolveInput('q1', answers);
  ...
  const cycle = resolveInput('q15', answers);
  const cycleMetric = benchmark.metrics.salesCycleDays;

  // Follow-up and pipeline leads the evidence rows, because a slow cycle is
  // still the loudest operational signal most respondents give us. It just
  // reports days against days now instead of an invented dollar figure. The
  // row is skipped when the cycle is untracked or the model's own median is
  // too short to be a coachable gap (a 2-day e-commerce "cycle").
  const evidenceRows = [];
  if (!cycle.notTracked && cycle.value && cycleMetric.median >= MIN_RESOLVABLE_CYCLE_DAYS) {
    const { interpretation } = classifyAgainstBenchmark(cycle.value, cycleMetric);
    if (interpretation !== 'meets') {
      evidenceRows.push({
        area: 'followupPipeline',
        areaTitle: AREA_TITLES.followupPipeline,
        kind: 'evidence',
        body: salesCycleBody(fmtDays(cycle.value), fmtDays(cycleMetric.median), benchmark.label),
        verdict: verdictFor('followupPipeline', answers, observed),
        fix: FIX_PARAGRAPHS.salesCycle,
        source: metricCitation(cycleMetric),
      });
    }
  }
