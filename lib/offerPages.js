...
/**
 * The metadata for the three offer pages, in one place.
 *
 ...
 * segment. These three pages are child segments, so a bare title string would
 * render 16 characters longer than it reads in the source. All three approved
 * titles are 52 or 53 characters, which the suffix would push to 68 or 69, well
 * past the roughly 60 characters Google shows. Three /learn titles sat
 * truncated for three weeks because that suffix was invisible in the source.
 *
 * So every page here opts out with `title: { absolute }`, exactly as
 * /learn/[slug] does, and the opt-out lives in one builder rather than in three
